...
create_mesh :: proc(vertex_positions : []f32, vertex_normals : []f32, elements : []u16) -> Mesh {
	mesh : Mesh
	...
	gl.BufferData(
		gl.ARRAY_BUFFER, 
		size_of(f32) * len(vertex_positions), 
	...
	gl.BufferData(
		gl.ARRAY_BUFFER,
		size_of(f32) * len(vertex_normals),
		raw_data(vertex_normals),
		gl.STATIC_DRAW,
	)

	gl.VertexAttribPointer(1, 3, gl.FLOAT, gl.FALSE, 3 * size_of(f32), uintptr(0))
